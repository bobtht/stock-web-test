// Cloudflare Worker - 台股 AI 分析網 (Phase 1 - 完整版)
// 包含：大盤/個股/AI選股/新聞 四個頁面 + 互動圖表
...
async function getStockData(symbol) {
  ...
  const basePrice = symbol === '2330.TW' ? 1865 : 
                   symbol === '2454.TW' ? 1720 : 
                   symbol === '0050.TW' ? 75 : 
                   symbol === '2317.TW' ? 214 : 100;
  ...
  return {
    symbol: symbol,
    name: stockNames[symbol] || '未知股票',
    price: price,
    open: price - Math.floor(Math.random() * 10),
    high: price + Math.floor(Math.random() * 20),
    low: price - Math.floor(Math.random() * 20),
    close: price,
    volume: Math.floor(Math.random() * 50000) + 10000,
    change: change,
    changePercent: ((change / basePrice) * 100).toFixed(2),
    ma5: price + Math.floor(Math.random() * 10 - 5),
    ma20: price + Math.floor(Math.random() * 20 - 10),
    rsi: Math.floor(Math.random() * 40) + 30,
    macd: (Math.random() * 2 - 1).toFixed(2),
    timestamp: new Date().toISOString(),
    candles: generateCandleData(60, basePrice)
  };
}
...
// 產生 K線資料
function generateCandleData(days, basePrice) {
  const data = [];
  let price = basePrice;
  const now = new Date();
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white rounded-2xl p-6 card-shadow hover-lift">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span id="taiex-badge" class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-bold">--</span>
                    </div>
                    <div class="text-4xl font-bold text-gray-800" id="taiex-value">--</div>
                    <div class="text-gray-500 mt-2" id="taiex-change">--</div>
                </div>
                <div class="bg-white rounded-2xl p-6 card-shadow hover-lift">
                    <div class="flex items-center justify-between mb-4">
    ...
    <script>
        ...
        async function loadMarketData() {
            try {
                ...
                const taiexChange = data.taiex.change;
                document.getElementById('taiex-change').textContent = (taiexChange > 0 ? '▲ ' : '▼ ') + Math.abs(taiexChange);
                document.getElementById('taiex-change').className = taiexChange > 0 ? 'text-red-500 mt-2' : 'text-green-500 mt-2';
                document.getElementById('taiex-badge').textContent = (data.taiex.changePercent > 0 ? '+' : '') + data.taiex.changePercent + '%';
                document.getElementById('taiex-badge').className = data.taiex.changePercent > 0 ? 'bg-red-100 text-red-600 px-3 py-1 rounded-full text-sm font-bold' : 'bg-green-100 text-green-600 px-3 py-1 rounded-full text-sm font-bold';
                
                document.getElementById('otc-value').textContent = data.otc.index.toFixed(2);
                const otcChange = data.otc.change;
                document.getElementById('otc-change').textContent = (otcChange > 0 ? '▲ ' : '▼ ') + Math.abs(otcChange);
                document.getElementById('otc-change').className = otcChange > 0 ? 'text-red-500 mt-2' : 'text-green-500 mt-2';
                document.getElementById('otc-badge').textContent = (data.otc.changePercent > 0 ? '+' : '') + data.otc.changePercent + '%';
                document.getElementById('otc-badge').className = data.otc.changePercent > 0 ? 'bg-red-100 text-red-600 px-3 py-1 rounded-full text-sm font-bold' : 'bg-green-100 text-green-600 px-3 py-1 rounded-full text-sm font-bold';
        ...
        function initMarketChart() {
            ...
            // 載入假資料
            const data = generateMockCandles(30, 22000);
        ...
        async function runScreener() {
            ...
            try {
                ...
                let html = '<div class="mb-4 flex items-center justify-between"><h3 class="text-lg font-bold text-gray-800">掃描完成，找到 ' + data.selected.length + ' 檔強勢股</h3><span class="text-sm text-gray-500">更新時間: ' + new Date(data.timestamp).toLocaleString() + '</span></div><div class="grid gap-4">';
                
                data.selected.forEach((stock, index) => {
                    html += '<div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 border border-indigo-100"><div class="flex items-center justify-between"><div class="flex items-center space-x-4"><div class="bg-indigo-500 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold">' + (index + 1) + '</div><div><div class="font-bold text-gray-800">' + stock.name + '</div><div class="text-sm text-gray-500">' + stock.symbol + '</div></div></div><div class="text-right"><div class="text-2xl font-bold text-gray-800">$' + stock.price + '</div><div class="text-sm text-green-600">評分: ' + stock.score + '</div></div></div><div class="mt-2 text-sm text-gray-600"><span class="bg-green-100 text-green-700 px-2 py-1 rounded">' + stock.reasons + '</span></div><div class="mt-2 text-xs text-gray-500">支撐: $' + stock.support + ' | 壓力: $' + stock.resistance + '</div></div>';
                });
        ...
        // 產生假資料
        function generateMockCandles(days, basePrice) {
            const data = [];
            let price = basePrice;
            const now = new Date();
